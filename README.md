...
在後台「設置」頁填入 Cap 實例地址與密鑰即啟用：

| 欄位 | 值 |
|------|-----|
| 人機驗證地址 | `https://cap.example.com/<site-key>/`（Cap 後台的 site key） |
| 人機驗證密鑰 | Cap 後台的 **secret key**（不是 ADMIN_KEY） |

- 位址與密鑰**兩項都填**才啟用；只填一項視為未啟用（避免拿空密鑰去問而全部失敗）。
- 密鑰**只留在服務端**，絕不下發瀏覽器；地址會回顯給前端（widget 需靠它取題）。
